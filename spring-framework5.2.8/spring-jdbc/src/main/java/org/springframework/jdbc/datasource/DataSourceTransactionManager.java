...
public class DataSourceTransactionManager extends AbstractPlatformTransactionManager
	...
	protected Object doGetTransaction() {
		...
		// 我们可以看到核心的就是 ConnectionHolder，这个对象尤为重要，注意，这里并没有建立连接，而且也可能获取到空的 connectionHolder，
		// 连接的包裹对象 ConnectionHolder 可以控制两个事物到底是不是用的同一个连接（同一个真正的事务，一起rollback 一起commit 的那种）。
	...
	protected void doBegin(Object transaction, TransactionDefinition definition) {
		...
		try {
			...
			if (!txObject.hasConnectionHolder() ||
				...
				// !txObject.hasConnectionHolder()  第一次创建事务为null，从数据源获取连接对象,点进去getConnection()。
				Connection newCon = obtainDataSource().getConnection();
				if (logger.isDebugEnabled()) {
					logger.debug("Acquired Connection [" + newCon + "] for JDBC transaction");
				}
				// 将连接包装成ConnectionHolder对象，放入当前事务对象中，第二个参数：true 表示是新建的ConnectionHolder
				txObject.setConnectionHolder(new ConnectionHolder(newCon), true);
			}
			// 这个会用来判断是否要创建新连接，就在getConnection那里判断的
			txObject.getConnectionHolder().setSynchronizedWithTransaction(true);
			// 获取连接，如果是第一次，是空，上面判断中就会把conn创建并且设置到事务对象中去，此时直接拿就好了
			con = txObject.getConnectionHolder().getConnection();
			// 修改一些连接属性，并且返回原有的隔离级别
			Integer previousIsolationLevel = DataSourceUtils.prepareConnectionForTransaction(con, definition);
			// 将连接原有的隔离级别存在事务对象中，方便连接归还给数据源时恢复现场
			txObject.setPreviousIsolationLevel(previousIsolationLevel);
			// 设置只读属性
			txObject.setReadOnly(definition.isReadOnly());

			// 连接对象，默认都是自动提交，给连接对象设置归还连接资源时需要手动提交的一个标识位
			...
			// 此方法内部会判断是否为只读事务，并创建Statement对象
			prepareTransactionalConnection(con, definition);
			// ConnectionHolder内包装了连接对象，设置该连接对象已经开启了事务，处于激活状态
			...
			if (txObject.isNewConnectionHolder()) {
				// 这里使用了ThreadLocal来保存了一个map对象，key为数据源，value为当前事务的ConnectionHolder，
				// 这是用来解决事务传播行为，可以通过从ThreadLocal内部获取连接来判断当前是否有事务开启
