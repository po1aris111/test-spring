...
public abstract class AbstractPlatformTransactionManager implements PlatformTransactionManager, Serializable {
	...
	public final TransactionStatus getTransaction(@Nullable TransactionDefinition definition)
		...
		/**
		 * 获取事务对象，这里是用来处理事务传播行为的
		 * 1.创建事务对象
		 * 2.设置事务对象是否支持保存点，默认不支持
		 * 3.从线程上下文ThreadLocal中获取ConnectionHolder，在第一次进入事务方法后，会将当前线程创建的ConnectionHolder存入ThreadLocal中
		 * 4.务创建的连接设置到新事务对象中，第二个参数：false  表示不是新建的连接，目前是和上一个事务共享的连接
		 */
		...
		if (def.getPropagationBehavior() == TransactionDefinition.PROPAGATION_MANDATORY) {
			...
			try {
				...
				resume(null, suspendedResources);// suspendedResources为空，也就没有回滚
				throw ex;
			}
		}
		else {
	...
	private TransactionStatus startTransaction(TransactionDefinition definition, Object transaction,
		...
		// getTransactionSynchronization()返回0   SYNCHRONIZATION_NEVER==2
		boolean newSynchronization = (getTransactionSynchronization() != SYNCHRONIZATION_NEVER);
		/**
		 * 第三个参数：true,表示是否为新建事务，根据已有的参数创建一个事务状态对象，suspendedResources：挂起事务的资源，此时为null
		 */
		DefaultTransactionStatus status = newTransactionStatus(
				definition, transaction, true, newSynchronization, debugEnabled, suspendedResources);
		// 开启事务，点进去
		doBegin(transaction, definition);
		// 设置线程上下文,把事务的信息都保存到ThreadLocal中
