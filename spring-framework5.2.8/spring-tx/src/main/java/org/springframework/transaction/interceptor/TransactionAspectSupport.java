...
public abstract class TransactionAspectSupport implements BeanFactoryAware, InitializingBean {
	...
	protected Object invokeWithinTransaction(Method method, @Nullable Class<?> targetClass,
		...
		// tas.getTransactionAttribute(method, targetClass)这个方法进去，会获取到方法上面的事务，并获取到注解里面配置的信息
		// 然后封装成TransactionAttribute返回
		final TransactionAttribute txAttr = (tas != null ? tas.getTransactionAttribute(method, targetClass) : null);
		// 获取事务管理器，一般是DataSourceTransactionManager类型
		...
		// 获取事务名称，优先取注解的描述信息，为null的话，用类的全限定名.方法名
		final String joinpointIdentification = methodIdentification(method, targetClass, txAttr);
		// 如果是声明式事务
		if (txAttr == null || !(ptm instanceof CallbackPreferringPlatformTransactionManager)) {
			// 根据事务管理器，事务属性和事务名称创建出事务信息对象，点进去，这个方法很长
			TransactionInfo txInfo = createTransactionIfNecessary(ptm, txAttr, joinpointIdentification);

			Object retVal;
			try {
				// 执行目标方法,invocation是最上面匿名内部类里重写的法，
				// 继续驱动下一个拦截器, 最终拦截器执行完成后，会执行目标方法
	...
	protected TransactionInfo prepareTransactionInfo(@Nullable PlatformTransactionManager tm,
		...
		/**
		 * tm：事务管理器txAttr：事务属性joinpointIdentification：事务名称
		 */
		...
		if (txAttr != null) {
			...
			// 保存status：事务的运行时信息基本都在这里面，比如连接信息，挂起的事务的信息（保存点），事务跑没跑完等等。
		...
		// 将TransactionInfo当前事务信息绑定到当前线程上下文
		txInfo.bindToThread();
		return txInfo;
	}
	...
	protected static final class TransactionInfo {
		...
		private void restoreThreadLocalStatus() {
			// 我们发现它这里有一个 ThreadLocal，它会将之前已经保存在这里的 TransactionInfo 拿出来，放到刚才上面提到的 old 里面持有
			// 而当前这个 TransactionInfo 对象则会扔进这个 ThreadLocal 里面，然后在执行完切面后把 old 放回去
